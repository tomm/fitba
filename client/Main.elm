    ...
    case error of
        Http.BadStatus response ->
            if response == 403 then
                ...
                ( { model | errorMsg = Just <| String.fromInt response }, Cmd.none )
...
update msg model =
    let
        updateState newState =
            ( { model | state = GameData newState, errorMsg = Nothing }, Cmd.none )

        updateStateCmd newState cmd =
            ( { model | state = GameData newState, errorMsg = Nothing }, cmd )

        updateWatchingGame watchingGame update2 =
            let
                game =
                    watchingGame.game
            in
            { watchingGame
                | game =
                    { game
                        | events = List.append game.events update2.events
                        , attending = update2.attending
                    }
            }

        handleLoadingStateMsgs =
            case msg of
                GotStartGameData result ->
                    case result of
                        Ok data ->
                            ( { model
                                | state =
                                    GameData
                                        { ourTeamId = data.team.id
                                        , tab = TabTeam { team = data.team, view = TeamViewTypes.SquadView { selectedPlayer = Nothing } }
                                        , ourTeam = data.team
                                        , season = data.season
                                        , currentTime = Time.millisToPosix 0
                                        , -- would be better not to wait for tick to resolve this...
                                          fixtures = []
                                        , news = []
                                        , topScorers = []
                                        , history = Nothing
                ...
                _ ->
                    ( { model | errorMsg = Just "Unexpected message while loading ..." }, Cmd.none )
        ...
        handleActiveStateMsgs m =
            case msg of
                    ...
                    let
                        cmd =
                            case tab of
                                ...
                                TabNews ->
                                    ClientServer.loadNews

                                TabHistory ->
                                    getHistory (m.season - 1)

                                _ ->
                                    Cmd.none
                    in
                    updateStateCmd { m | tab = tab } cmd

                ViewTransferMarket ->
                    ( model, ClientServer.loadTransferListings )

                Watch gameId ->
                    ( model, ClientServer.loadGame gameId )

                GotNews result ->
                    case result of
                        Ok news ->
                            updateState { m | news = news }

                        Err error ->
                            handleHttpError error model

                GotTransferListings result ->
                    case result of
                        Ok listings ->
                            updateState
                                { m
                                    | tab =
                                        TabTransferMarket
                                            { view = TransferMarketTypes.ListView
                                            , listings = listings
                                            }
                                }
                ...
                ViewTeam teamId ->
                    ( model, ClientServer.loadTeam teamId )

                ViewTeamLoaded result ->
                    case result of
                        Ok team ->
                            updateState
                                { m
                ...
                SecondTick t ->
                    let
                        ( state, cmd ) =
                            MatchView.update MatchViewMsg.GameTick m

                        state2 =
                            { state | currentTime = t }
                    in
                    ( { model | state = GameData state2 }, cmd )
                ...
                MsgTeamView msg2 ->
                    case m.tab of
                        TabTeam state ->
                            let
                                ( newState, cmd ) =
                                    TeamView.update msg2 state
                            in
                            updateStateCmd
                                { m
                                    | tab = TabTeam newState
                                    , ourTeam = newState.team
                                }
                                cmd
                ...
                MsgMatchView msg2 ->
                    let
                        ( state, cmd ) =
                            MatchView.update msg2 m
                    in
                    ( { model | state = GameData state }, cmd )

                MsgTransferMarket msg2 ->
                    case m.tab of
                        TabTransferMarket state ->
                            let
                                ( newState, cmd ) =
                                    TransferMarket.update msg2 state
                            in
                            updateStateCmd { m | tab = TabTransferMarket newState } cmd
                ...
                UpdateFixtures result ->
                    case result of
                        Ok fixtures ->
                            updateState { m | fixtures = fixtures }
                    ...
                    case result of
                        Ok game ->
                            if game.status == InProgress then
                                ...
                                let
                                    timeElapsed =
                                        List.length game.events

                                    -- XXX TODO hack. will only work
                                ...
                                updateState
                                    { m
                                        | tab =
                                            TabMatch
                                                { game = game
                                                , timePoint = Time.millisToPosix timeElapsed
                                                }
                                    }

                            else
                                -- start Played and Scheduled games from beginning
                                updateState { m | tab = TabMatch { game = game, timePoint = Time.millisToPosix 0 } }
                        ...
                        Ok update2 ->
                            case m.tab of
                                TabMatch watchingGame ->
                                    updateState
                                        { m
                                            | tab = TabMatch (updateWatchingGame watchingGame update2)
                                        }
                ...
                UpdateHistory result ->
                    case result of
                        Ok history ->
                            updateState { m | history = Just history }
                ...
                UpdateLeagueTables result ->
                    case result of
                        Ok tables ->
                            updateState { m | leagueTables = tables }

                        Err error ->
                            handleHttpError error model

                UpdateTopScorers result ->
                    case result of
                        Ok scorers ->
                            updateState { m | topScorers = scorers }
                ...
                GotStartGameData result ->
                    case result of
                        Ok data ->
                            updateState { m | ourTeam = data.team, season = data.season }
                ...
                DeleteInboxMessage msgId ->
                    let
                        team =
                            m.ourTeam
                    in
                    updateStateCmd { m | ourTeam = { team | inbox = List.filter (\item -> item.id /= msgId) team.inbox } }
    ...
    let
        ...
        clubTabLabel =
            case List.length model.ourTeam.inbox of
                ...
                n ->
                    "(" ++ String.fromInt n ++ ") Club"

        tabLabels =
            [ ( TabTeam { team = model.ourTeam, view = TeamViewTypes.SquadView { selectedPlayer = Nothing } }, "Squad" )
...
view model =
    div []
        [ case model.state of
            Loading ->
                text <| Maybe.withDefault "Loading ..." model.errorMsg
            ...
            GameData m ->
                div []
                    ...
                    , case m.tab of
                        TabViewOtherTeam state ->
                            Html.map (\_ -> NoOp {- can't edit -}) <| TeamView.view state

                        TabTeam state ->
                            Html.map MsgTeamView <| TeamView.view state
                        ...
                        TabFixtures subview ->
                            FixturesView.view m subview

                        TabMatch watchingGame ->
                            Html.map MsgMatchView <| MatchView.view watchingGame

                        TabClub ->
                            clubTab m

                        TabTransferMarket state ->
                            Html.map MsgTransferMarket <| TransferMarket.view m.ourTeamId state
...
topScorerView model =
    let
        style scorer =
            if Just model.ourTeam.name == scorer.teamName then
                [ Html.Attributes.class "scorer-own" ]

            else
                []

        topScorerTable tournamentTopScorers =
...
tournamentTab model =
    Uitk.view Nothing (text <| "Season " ++ String.fromInt model.season) <|
        [ Uitk.blockButtonRow
            [ Uitk.column 8 []
...
clubTab model =
    let
        numMsgs =
            "(" ++ String.fromInt (List.length model.ourTeam.inbox) ++ ")"
    in
    Uitk.view Nothing
        (text "Club")
        [ Html.p [ class "center" ]
