...
blockButtonRow columns =
    div [ class "clicky-clicky" ] [ row columns ]
...
actionButton action label =
    button [ class "pure-button", class "button-action", onClick action ] [ text label ]


infoButton : a -> Html a
infoButton action =
    button [ class "pure-button", class "button-info", onClick action ] [ text infoIcon ]


backButton : a -> Html a
backButton action =
    button [ class "pure-button", class "button-action", onClick action ] [ text "Back" ]


crossButton : a -> Html a
crossButton action =
    button [ class "icon-button", onClick action ]
...
playerPositionBadge player =
    let
        pos =
            Types.playerPositionFormat player.positions
    in
    Html.span [ class "player-position-badge", class <| "player-position-" ++ pos ] [ text pos ]
...
row columns =
    Html.div [ class "pure-g" ] (List.map unpackRowColumn columns)


column : Int -> List (Html a) -> RowColumn a
column num24ths contents =
    RowColumn <| Html.div [ class <| "pure-u-" ++ String.fromInt num24ths ++ "-24" ] contents


responsiveColumn : Int -> List (Html a) -> RowColumn a
responsiveColumn num24ths contents =
    RowColumn <| Html.div [ class "pure-u-1", class <| "pure-u-md-" ++ String.fromInt num24ths ++ "-24" ] contents
