module TransferMarket exposing (update, view)
            ...
            let
                bidInput =
                    div []
                        ...
                        , Uitk.row
                            [ Uitk.column 24
                                [ input
                                    [ type_ "number"
                                    , step "10000"
                                    , value <| String.fromInt pvstate.bidInputValue
                                    , onInput <|
                                        UpdateBidInput
                                    ]
                                    []
                        ...
                        , Uitk.row
                            [ Uitk.column 6 [ Uitk.actionButton (UpdateBidInput <| String.fromInt <| pvstate.bidInputValue + 10000) "+" ]
                            , Uitk.column 6 [ Uitk.actionButton (UpdateBidInput <| String.fromInt <| pvstate.bidInputValue - 10000) "-" ]
                            ]
                        ]

                withdrawButton =
                    div [] [ Uitk.actionButton WithdrawBid "Withdraw bid" ]
            in
            Uitk.view (Just <| Uitk.backButton ViewAll)
                (text "Transfer Listing")
                [ Uitk.row
                    ...
                    , Uitk.column 11
                        [ case pvstate.listing.status of
                            ...
                            Sold ->
                                div [] [ text "This player has been sold." ]

                            Unsold ->
                                div [] [ text "No winning bid." ]

                            YouWon ->
                                div [] [ text "You have signed this player." ]

                            OutBid ->
                                div [] [ text "You were outbid by another team." ]

                            TeamRejected ->
                                div [] [ text "The seller rejected your offer." ]

                            PlayerRejected ->
                                div [] [ text "The player was not interested in playing for your team." ]

                            InsufficientMoney ->
                                div [] [ text "You did not have enough money to conclude the deal." ]
                        ]
                    ]
                ]
                        ...
                        , Html.td []
                            [ text <|
                                case listing.status of
                                    OnSale ->
                                        Utils.timeFormatShort listing.deadline
                        ...
                        , Html.td []
                            [ text <|
                                if listing.sellerTeamId == ownTeamId then
                                    ...
                                    case listing.youBid of
                                        Just amount ->
                                            Utils.moneyFormat amount

                                        Nothing ->
                                            ""
                            ]
                        , Html.td [] [ text <| Types.playerAvgSkill listing.player ]
                        , Html.td [] [ text <| String.fromInt <| listing.player.shooting ]
                        , Html.td [] [ text <| String.fromInt <| listing.player.passing ]
                        , Html.td [] [ text <| String.fromInt <| listing.player.tackling ]
                        , Html.td [] [ text <| String.fromInt <| listing.player.handling ]
                        , Html.td [] [ text <| String.fromInt <| listing.player.speed ]
                        ]
            in
            Uitk.view Nothing
...
update msg state =
    case msg of
        WithdrawBid ->
            case state.view of
                PlayerView pvstate ->
                    ( withdrawBid state pvstate.listing
        ...
        MakeBid ->
            case state.view of
                PlayerView pvstate ->
                    ( makeBid state pvstate.bidInputValue pvstate.listing
        ...
        UpdateBidInput maybeAmount ->
            case state.view of
                PlayerView pvstate ->
                    case String.toInt maybeAmount of
                        Just amount ->
                            ( { state | view = PlayerView { pvstate | bidInputValue = Basics.max pvstate.listing.minPrice amount } }, Cmd.none )
                        Nothing ->
                            ( { state | view = PlayerView pvstate }, Cmd.none )
        ...
        -}
        ViewAll ->
            ( { state | view = ListView }, Cmd.none )

        ViewListing listing ->
            ( { state | view = PlayerView { listing = listing, bidInputValue = listing.minPrice } }, Cmd.none )
