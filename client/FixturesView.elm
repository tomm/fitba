module FixturesView exposing (resultText, view)
...
resultText fixture =
    case fixture.status of
        ...
        Played result ->
            text
                (String.fromInt result.homeGoals
                    ++ " : "
                    ++ String.fromInt result.awayGoals
                    ++ (if result.homePenalties > 0 || result.awayPenalties > 0 then
                            " ("
                                ++ String.fromInt result.homePenalties
                                ++ " : "
                                ++ String.fromInt result.awayPenalties
                                ++ " P)"
...
view model subView =
    let
        fixtureRow showTournament timeFormatter fixture =
            tr
                ...
                [ td []
                    [ if showTournament then
                        ...
                        case fixture.stage of
                            ...
                            Just stage ->
                                span [ Html.Attributes.class "fixture-tournament" ] [ text <| cupStage stage ]
        ...
        tournaments =
            Set.toList <| Set.fromList (List.map .tournament model.fixtures)
        ...
        todaysFixtures =
            [ Uitk.blockButtonRow <|
                List.map
                    ...
                    tournaments
            , fixtureTable True
                Utils.onlyTimeFormat
                "Today's Matches"
                (List.filter (\f -> Utils.dateEq f.start model.currentTime) model.fixtures)
            ]
    in
